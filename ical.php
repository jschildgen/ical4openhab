...
if(count($argv) < 2 || !($argv[1] == 'DTSTART' || $argv[1] == 'DTEND')) {
  echo "Usage: php /path/to/ical.php DTSTART|DTEND [URL] [Refresh] [Offset] [Filter]";
  die();
}
...
require_once('/home/openhabian/ical/class.iCalReader.php');
...
$exception_list = find_exceptions($ical);

foreach($ical->events() as $event) {
  //print_r($event); continue;
  debug("===> ".$event["SUMMARY"]."      (".@$event["UID"].")");

  if($filter != null && stripos($event["SUMMARY"],$filter)===FALSE) {
    debug("Filtered");
    continue;
  }
...
function find_exceptions($ical) {
  $res = array();
  foreach($ical->events() as $event) {
    if(isset($event["RECURRENCE-ID"])) {
	    $res[$ical->iCalDateToUnixTimestamp($event["RECURRENCE-ID"])] = true;
    }
  }
  return $res;
}
